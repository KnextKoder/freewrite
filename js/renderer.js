...
const fontButton = requireElement("font-btn");
const fontSizeButton = requireElement("font-size-btn");
const systemFontButton = requireElement("system-font-btn");
const serifFontButton = requireElement("serif-font-btn");
const randomFontButton = requireElement("random-font-btn");
const timerButton = requireElement("timer-btn");
const themeButton = requireElement("theme-btn");
const fullscreenButton = requireElement("fullscreen-btn");
const newEntryButton = requireElement("new-entry-btn");
const chatButton = requireElement("chat-btn");
const historyButton = requireElement("history-btn");
const entriesList = requireElement("entries-list");
const fontSizePopup = requireElement("font-size-popup");
const sidebar = requireElement("sidebar");
const closeSidebarButton = requireElement("close-sidebar-btn");
...
ipcRenderer.on("fullscreen-state", (_event, isFullScreen) => {
    if (isFullScreen) {
...
const loadEntries = () => {
    ...
    ipcRenderer.on("entries-loaded", (_event, data) => {
...
ipcRenderer.on("entry-loaded", (_event, data) => {
    if (!data.success) {
        return;
    }
...
ipcRenderer.on("welcome-message-loaded", (_event, data) => {
    if (!data.success) {
        return;
    }
    editor.value = `\n\n${data.content ?? ""}`;
...
const showCustomAlert = (message, callback) => {
  ...
  `;
    ...
    okButton?.focus();
    alertBox.addEventListener("keydown", (event) => {
